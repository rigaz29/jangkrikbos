...
const toolMap = {
  ...
  update_config: ({ changes, reason = "" }) => {
    ...
    const CONFIG_MAP = {
      ...
      maxAthDropPct:    ["screening", "maxAthDropPct"],
    ...
    const intervalChanged = applied.managementIntervalMin != null || applied.screeningIntervalMin != null || applied.healthCheckIntervalMin != null;
    ...
    if (success) {
      if (name === "swap_token" && result.tx) {
        ...
        notifyDeploy({
          ...
          feeTvlRatio: result.fee_tvl_ratio ?? args.fee_tvl_ratio,
          organicScore: result.organic_score ?? args.organic_score,
          downsidePct: result.downside_pct,
        }).catch(() => {});
        ...
        notifyClose({
          ...
          minutesInRange: result.minutes_in_range,
          strategy: result.strategy,
          pool: result.pool,
