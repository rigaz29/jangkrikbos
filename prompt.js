...
- STRATEGY IS AUTO-DECIDED FROM VOLATILITY — you do NOT pick the shape:
  * volatility >= ${config.strategy.volatilityStrategyThreshold ?? 2.5} → bid_ask (high-vol: wide, SOL-side, OOR-resilient, accumulate dips)
  * volatility <  ${config.strategy.volatilityStrategyThreshold ?? 2.5} → spot (calmer: two-sided fee farming)
  Always pass the candidate's volatility to deploy_position so the shape is set correctly. You may omit strategy — volatility overrides it.
- Momentum/quality signals (smart_money_buy, narrative, net_buyers, age) decide WHETHER the token is worth deploying and its risk — NOT the strategy shape.
- bins_below and bins_above are auto-calculated — DO NOT pass them. Always pass bin_step.
